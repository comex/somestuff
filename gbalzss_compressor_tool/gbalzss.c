	   ...
	   Note r plays double role, as tree node and position in buffer. */
{
	...
	cmp = 1;  key = &text_buf[r];  p = N + 1 + (key[0] == 0xffff ? 0 : key[0]);
...
// function patched to produce gba bios decompression function processable data
{
	...
	code_buf_ptr = 1;
	mask = 0x80;	// f�r GBA fangen wir mit MSB an
	s = 0;
	r = N - F; 	// 4078

	for (i = s; i < r; i++) text_buf[i] = 0xffff;  /* Clear the buffer with any character that will appear often. */
