...
class DOL(object):
    ...
    def getString(self, addr, length):
        for (addr1, ware, _) in self.text + self.data:
            if addr >= addr1 and addr < (addr1 + len(ware) - length):
                return ware[addr - addr1:addr - addr1 + length]
